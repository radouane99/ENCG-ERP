...
class EncgFesSeeder extends Seeder
{
    ...
    private function seedMoroccanStudentsAndGrades(
        ...
        $studentProfile = Student::create([
            ...
            'blood_type' => 'O+',
            'father_name' => 'Hassan El Mansouri',
            'father_name_ar' => 'حسن المنصوري',
            'father_cin' => 'F124578',
            'father_profession' => 'Cadre Bancaire',
            'father_phone' => '+212 661 123456',
            'mother_name' => 'Khadija Bennani',
            'mother_name_ar' => 'خديجة بناني',
            'mother_cin' => 'F236589',
            'mother_profession' => 'Professeure de Lycée',
            'mother_phone' => '+212 662 654321',
            'emergency_contact_name' => 'Hassan El Mansouri',
            'emergency_contact_phone' => '+212 661 123456',
        ...
        // 2. Complete Moroccan names and profiles bank
        $names = [
            [
        ...
        foreach ($filiereSet as $set) {
            ...
            for ($g = 1; $g <= 2; $g++) {
                ...
                for ($s = 0; $s < $countToAdd; $s++) {
                    ...
                    $birthDate = '2004-'.str_pad(rand(1, 12), 2, '0', STR_PAD_LEFT).'-'.str_pad(rand(1, 28), 2, '0', STR_PAD_LEFT);
                    $cinNumber = $nameData['cin_prefix'].rand(20000, 99999);
                    $cneCode = 'N13'.str_pad($studentIndex, 7, '0', STR_PAD_LEFT);
                    $region = match ($nameData['city']) {
                        'Rabat', 'Kénitra' => 'Rabat-Salé-Kénitra',
                    ...
                    $fatherPhone = '+212 66'.rand(1, 9).' '.rand(100000, 999999);
                    $motherPhone = '+212 66'.rand(1, 9).' '.rand(100000, 999999);
                    ...
                    $student = Student::create([
                        ...
                        'gender' => $gender,
                        'status' => 'active',
                        'first_name_ar' => $nameData['first_ar'],
                        'last_name_ar' => $nameData['last_ar'],
                        'cin' => $cinNumber,
                        'birth_date' => $birthDate,
                        'birth_city' => $nameData['city'],
                        'birth_city_ar' => $nameData['city_ar'],
                        'birth_country' => 'MA',
                        'nationality' => 'Marocaine',
                        'address' => $nameData['address'],
                        'city' => $nameData['city'],
                        'region' => $region,
                        'postal_code' => $nameData['postal_code'],
                        'high_school' => $nameData['lycee'],
                        'lycee' => $nameData['lycee'],
                        'academy' => 'AREF '.$region,
                        'delegation' => 'Direction Provinciale '.$nameData['city'],
                        'province' => $nameData['city'],
                        'bac_year' => $nameData['bac_year'],
                        'bac_type' => $nameData['bac_type'],
                        'bac_serie' => $nameData['bac_type'],
                        'blood_type' => $nameData['blood_type'],
                        'father_name' => $nameData['father_name'],
                        'father_name_ar' => $nameData['father_name_ar'],
                        'father_cin' => $nameData['cin_prefix'].rand(10000, 99999),
                        'father_profession' => $nameData['father_job'],
                        'father_phone' => $fatherPhone,
                        'mother_name' => $nameData['mother_name'],
                        'mother_name_ar' => $nameData['mother_name_ar'],
                        'mother_cin' => $nameData['cin_prefix'].rand(10000, 99999),
                        'mother_profession' => $nameData['mother_job'],
                        'mother_phone' => $motherPhone,
                        'emergency_contact_name' => $nameData['father_name'],
                        'emergency_contact_phone' => $fatherPhone,
                        'emergency_contact_relation' => 'Père',
                    ]);
    ...
    private function seedLibrary(Institution $institution, User $studentUser): void
    {
        ...
        foreach ($booksData as $b) {
            ...
            for ($c = 1; $c <= 3; $c++) {
                ...
                if (! $isAvail) {
                    $scolariteId = User::where('email', 'scolarite@encg.ma')->value('id') ?? $studentUser->id;
                    DB::table('borrowings')->insert([
                        'book_copy_id' => $copyId,
                        'user_id' => $studentUser->id,
                        'issued_by' => $scolariteId,
    ...
    private function seedConvocationsAndSurveillance(Institution $institution, array $students, array $professors, array $rooms): void
    {
        ...
        foreach ($exams as $eIndex => $exam) {
            ...
            foreach ($registrations as $sIndex => $reg) {
                // Find student profile and user
                $studentProfile = Student::find($reg->student_id);
                if (! $studentProfile) {
                    continue;
                }
    ...
    private function seedHRVacations(
        ...
        $adminId = User::where('email', 'admin@encg.ma')->value('id') ?? 1;
    ...
    private function seedAttendancesAndJustifications(
        ...
        $adminId = User::where('email', 'admin@encg.ma')->value('id') ?? 1;
        $scolariteId = User::where('email', 'scolarite@encg.ma')->value('id') ?? $adminId;
        ...
        foreach ($groups as $group) {
            ...
            $groupStudentProfileIds = StudentRegistration::where('group_id', $group->id)->pluck('student_id')->toArray();
            if (empty($groupStudentProfileIds)) {
                continue;
            }
            ...
            for ($sessIdx = 1; $sessIdx <= 3; $sessIdx++) {
                ...
                foreach ($groupStudentProfileIds as $index => $sId) {
                    ...
                    if ($status === 'absent' && $isJust) {
                        DB::table('absence_justifications')->insert([
                            ...
                            'reviewed_by' => rand(1, 3) === 1 ? null : $scolariteId,
    ...
    private function seedConversationsAndMessages(
        ...
        $profUser = User::find($professors[0]->user_id) ?? $adminUser;
    ...
    private function seedDisciplinaryCasesAndDecisions(
        ...
        $adminUser = User::where('email', 'admin@encg.ma')->first();
        $student2Profile = Student::where('user_id', $students[2]->id)->first() ?? Student::first();
        $student4Profile = Student::where('user_id', $students[4]->id)->first() ?? Student::first();
        $prof0User = User::find($professors[0]->user_id);
        $prof1User = User::find($professors[1]->user_id);
        ...
        $case1Id = DB::table('disciplinary_cases')->insertGetId([
            ...
            'reported_by_name' => $prof1User?->name ?? 'Professeur',
            'status' => 'closed',
        ...
        DB::table('disciplinary_cases')->insert([
            ...
            'reported_by_name' => $prof0User?->name ?? 'Professeur',
            'status' => 'pending',
    ...
    private function seedModulePvSignatures(
        ...
        foreach ($modules as $i => $module) {
            ...
            $sigData = json_encode([
                'professor' => $profUser?->name ?? 'Professeur',
                'module' => $module->name,
                'group' => $group->name,
    ...
    private function seedNotifications(array $students, array $professors): void
    {
        ...
        $rows[] = [
            ...
            'notifiable_type' => 'App\Models\User',
            'notifiable_id' => $professors[0]->user_id,
    ...
    private function seedResearchLabsAndPublications(
        ...
        $prof0UserId = $professors[0]->user_id;
        $prof1UserId = $professors[1]->user_id;
