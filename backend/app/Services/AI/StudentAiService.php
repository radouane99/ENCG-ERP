...
class StudentAiService
{
    ...
    public function simulateGrade(int $studentId, float $targetExamGrade = 12.0): array
    {
        ...
        $isEliminatory = LmdRules::isEliminatory($targetExamGrade);
        $verdict = $isEliminatory ? 'NV' : LmdRules::decisionFromScore($simulatedAvg);
        if (! $isEliminatory && $simulatedAvg >= LmdRules::VALIDATION_THRESHOLD && $targetExamGrade < LmdRules::VALIDATION_THRESHOLD) {
            $verdict = 'VC';
        }
        ...
        return [
            ...
            'validation_status' => match ($verdict) {
                'V' => 'VALIDÉ (V)',
                'VC' => 'VALIDÉ PAR COMPENSATION (VC)',
                'NV' => 'NON VALIDÉ (NV)',
                default => 'RATTRAPAGE (RAT)',
            },
            'eliminatory_risk' => $isEliminatory
                ? 'ATTENTION : Note éliminatoire (< '.LmdRules::ELIMINATORY_THRESHOLD.'/20)'
                : 'Aucun risque éliminatoire',
            'ai_advice' => $verdict === 'V' || $verdict === 'VC'
                ? 'Félicitations ! Ce score vous garantit la validation du semestre.'
                : 'Objectif trop juste. Visez au moins '.LmdRules::VALIDATION_THRESHOLD.'/20 à l\'épreuve finale pour valider sans rattrapage.',
        ];
    }
