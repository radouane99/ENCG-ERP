...
class OfficialTimetableMatrixService
{
    ...
    public function catalog(Collection $schedules, array $meta = []): array
    {
        ...
        usort($sections, fn (array $a, array $b) => [$a['filiere_code'] ?? '', $a['semester_number'] ?? 0]
            <=>
    ...
    public function fromSchedules(Collection $schedules, array $meta = []): array
    {
        ...
        usort($rows, fn (array $a, array $b) => [$a['module_index'], $a['session_type'] === 'cm' ? 0 : 1, $a['professor_name']]
            <=>
        ...
        $semester = $meta['semester']
            ...
            ?? ($year ? Semester::where('academic_year_id', $year->id)->where('is_current', true)->first() : null);
