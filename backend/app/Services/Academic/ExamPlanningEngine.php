...
class ExamPlanningEngine
{
    ...
    public function generatePlan(int $examId, array $roomIds, array $professorIds, ?int $secondaryGroupId = null): array
    {
        ...
        if ($studentsB->isNotEmpty()) {
            ...
            for ($i = 0; $i < $maxCount; $i++) {
                if (isset($studentsA[$i])) $students->push($studentsA[$i]);
                if (isset($studentsB[$i])) $students->push($studentsB[$i]);
            }
        } else {
            $students = $studentsA;
        }
        ...
        return DB::transaction(function () use ($examId, $rooms, $students, $professorIds) {
            ...
            foreach ($rooms as $room) {
                if ($profIndex >= count($professorIds)) break;
    ...
    public function autoGenerateIntelligentBatch(
        ...
        if ($modules->isEmpty()) throw new Exception("Aucun module trouvé.");
        if ($groups->isEmpty()) throw new Exception("Aucun groupe trouvé.");
        ...
        return DB::transaction(function () use (
            ...
            if ($professors->isEmpty()) $professors = User::limit(5)->get();
            if ($vacataires->isEmpty()) $vacataires = User::limit(5)->get();
