...
class ReinscriptionController extends Controller
{
    ...
    public function getStatus(Request $request): JsonResponse
    {
        ...
        $filiere = $currentPathway?->filiere;
        $requiresPayment = $filiere?->requiresTuitionPayment() ?? false;
        $paymentStatus = $requiresPayment ? 'pending' : 'not_applicable';

        $filieres = ($isAdmis && in_array($targetSemester, [5, 7], true))
            ? Filiere::select('id', 'name', 'code', 'type')->get()
            : collect();
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'current_filiere' => $filiere?->name ?? 'Tronc Commun',
                ...
                'filieres' => $filieres,
                'requires_filiere_choice' => $isAdmis && in_array($targetSemester, [5, 7], true),
    ...
    public function confirm(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'payment_status' => 'nullable|in:paid,pending,failed,not_applicable',
            'payment_provider' => 'nullable|in:cmi,wafacash,simulation',
        ]);
        ...
        return DB::transaction(function () use ($student, $user, $validated, $currentYear, $request) {
            ...
            $targetFiliere = Filiere::find($targetFiliereId);

            if ($targetFiliere?->requiresTuitionPayment()) {
                $paid = ($validated['payment_status'] ?? null) === 'paid';
                if (! $paid) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Paiement requis (licence / master / formation continue). Simulation CMI ou Wafacash.',
                        'payment_required' => true,
                    ], 402);
                }
            }

            if ($currentPathway && ! in_array($targetSemester, [5, 7], true) && ! empty($validated['filiere_id']) && (int) $validated['filiere_id'] !== (int) $currentPathway->filiere_id) {
                return response()->json([
                    'success' => false,
                    'message' => 'Le choix de filière n\'est ouvert qu\'en S5 ou S7.',
                ], 422);
            }
    ...
    {
        $validated = $request->validate([
            'provider' => 'required|in:cmi,wafacash,simulation',
        ]);

        $student = Student::with('latestPathway.filiere')->where('user_id', $request->user()->id)->firstOrFail();
        $filiere = $student->latestPathway?->filiere;
        abort_unless($filiere?->requiresTuitionPayment(), 422, 'Aucun paiement pour le cursus Grande École.');

        return response()->json([
            'success' => true,
            'payment_status' => 'paid',
            'provider' => $validated['provider'],
            'message' => 'Paiement simulé (CMI / Wafacash).',
        ]);
    }
