...
class ProfessorAiCopilotController extends Controller
{
    ...
    public function generateExamPaper(Request $request): JsonResponse
    {
        ...
        if (!$apiKey) {
            return response()->json([
                'success' => true,
                'source'  => 'internal_engine',
                'data'    => $this->generateFallbackExamPaper($module, $typeLabel, $instructions),
            ]);
        }
        ...
        try {
            $prompt = "Tu es un Professeur Universitaire à l'ENCG Fès (Université Sidi Mohamed Ben Abdellah). "
                ...
                . "Format: {$typeLabel}. Niveau: {$validated['difficulty']}. Consignes particulières: {$instructions}. "
    ...
    private function generateFallbackExamPaper(Module $module, string $typeLabel, string $instructions): array
    {
        return [
            'title'        => "Épreuve Finale Officielle : {$module->name}",
            'context'      => "Le Groupe ATLAS Commerce, leader marocain de la distribution et de la gestion des opérations, fait face à une transformation majeure de ses processus managériaux. Vous êtes mandaté(e) en tant qu'expert-conseil diplômé de l'ENCG Fès pour analyser la situation et formuler des recommandations stratégiques.",
            'total_points' => 20,
            'sections'     => [
                [
