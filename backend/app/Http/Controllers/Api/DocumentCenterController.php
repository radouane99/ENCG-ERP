...
class DocumentCenterController extends Controller
{
    ...
    public function downloadDocument(Request $request, string $type, int $id)
    {
        ...
        if (!$token) abort(400, 'Token de téléchargement manquant.');
        ...
        return match (true) {
            in_array($type, ['attestation_scolarite', 'attestation', 'scolarite'])
                => $this->downloadStudentPdf($id, 'ATTESTATION DE SCOLARITÉ', 'Attestation_Scolarite', 'pdf.attestation_officielle', $commonData),

            in_array($type, ['attestation_reussite', 'reussite'])
                => $this->downloadStudentPdf($id, 'ATTESTATION DE RÉUSSITE', 'Attestation_Reussite', 'pdf.attestation_officielle', $commonData),

            in_array($type, ['attestation_decharge', 'decharge'])
                => $this->downloadStudentPdf($id, null, 'Attestation_Decharge', 'pdf.attestation_decharge', $commonData),

            in_array($type, ['attestation_vacations', 'vacations'])
                => $this->downloadProfessorPdf($id, 'pdf.attestation_vacations', 'Attestation_Vacations', $commonData),

            in_array($type, ['attestation_travail', 'travail'])
                => $this->downloadProfessorPdf($id, 'pdf.attestation_travail', 'Attestation_Travail', $commonData),

            in_array($type, ['convention_stage', 'stage', 'convention'])
                => $this->downloadConventionStagePdf($id, $commonData),
     ...
     */
    private function downloadStudentPdf(int $id, ?string $typeLabel, string $prefix, string $view, array $commonData): \Symfony\Component\HttpFoundation\Response
    {
        ...
        if ($typeLabel) $data['type'] = $typeLabel;
     ...
     */
    private function downloadProfessorPdf(int $id, string $view, string $prefix, array $commonData): \Symfony\Component\HttpFoundation\Response
    {
     ...
     */
    private function downloadConventionStagePdf(int $id, array $commonData): \Symfony\Component\HttpFoundation\Response
    {
        $student = Student::with(['user', 'latestPathway.filiere'])->findOrFail($id);
