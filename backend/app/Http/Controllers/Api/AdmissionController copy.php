...
class AdmissionController extends Controller
{
    ...
    /**
     * Upload candidate scanned document (Bac, CNIE, Photo, Relevé de Notes) securely.
     */
    public function uploadCandidateDocument(Request $request): JsonResponse
    {
        // 1. Strict MIME-type & extension validation to block malicious uploads (PHP shell scripts, executables, etc.)
        $request->validate([
            'file' => [
                'required',
                'file',
                'max:10240', // Limit to 10MB
                function ($attribute, $value, $fail) {
                    ...
                    if (!in_array($mime, $allowedMimes)) {
                        $fail("Le format MIME '{$mime}' n'est pas autorisé pour les documents officiels.");
                    }
            ...
            'type' => 'required|string|in:bac,cnie,photo,releve_notes,cin,cin_recto_verso,bac_pdf,releve_notes_pdf',
            'cne' => 'nullable|string',
            'cin' => 'nullable|string',
        ]);
        ...
        // 2. Store on private 'local' disk instead of 'public' directory
        $path = $file->store('private_candidate_documents', 'local');
        
        // 3. Generate secure authenticated serving URL
        $url = url("/api/documents/serve/{$type}/" . ($cne ?: 'unknown'));

        // Upsert into student_documents table with the private storage path
        DB::table('student_documents')->updateOrInsert(
            ...
            [
                'file_path' => $path, // Private path stored (e.g. private_candidate_documents/uuid.pdf)
        ...
        // If type is photo, update photo_path on application and student table
        if ($type === 'photo') {
            if ($cne) {
        ...
        return response()->json([
            'success' => true,
            'message' => "✅ Document '{$type}' téléversé et enregistré en privé avec succès !",
            'file_path' => $url,
        ]);
    }

    /**
     * Serve a privately stored candidate document securely with authentication and authorization checks.
     */
    public function serveCandidateDocument(Request $request, string $type, string $cne): \Symfony\Component\HttpFoundation\BinaryFileResponse|JsonResponse
    {
        ...
        // 3. Find the document record in PostgreSQL
        ...
        if (!$document) {
            return response()->json([
                'success' => false,
                'message' => 'Document introuvable en base de données.'
            ], 404);
        }

        // 4. Verify that the file actually exists on local storage disk
        ...
        // 5. Stream the file securely (does not expose public URLs)
