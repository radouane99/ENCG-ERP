...
<body>
    ...
    <!-- ========================================================= -->
    <!-- PAGE 1: BORDEREAU D'ÉMARGEMENT & DE TRANSMISSION DU LOT   -->
    <!-- ========================================================= -->
    <div class="page-container">
            ...
            <tr>
                ...
                <td style="width: 50%; text-align: center;">
                    ...
                    <div style="font-size: 6.2pt; font-weight: bold; color: #854d0e; text-transform: uppercase; letter-spacing: 0.6px; margin-top: 1px;">
                        Secrétariat Général — Service des Examens & Discipline
                    </div>
                </td>
                <td style="width: 25%; text-align: right;">
                    <div style="font-size: 6.5pt; color: #64748b; font-family: monospace;">
                        <strong>BORDEREAU LOT N° :</strong><br>
                        ENCG/BORD-CD-2026/LOT<br>
                        Édité le : {{ $generatedAt ?? date('d/m/Y') }}
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class="title-table">
            <tr>
                <td class="title-cell">
                    <h1>BORDEREAU D'ÉMARGEMENT & RÉCEPTION DES CONVOCATIONS</h1>
                    <div class="sub">Registre Officiel de Notification & Remise des Convocations Disciplinaires</div>
                </td>
            </tr>
        </table>

        <div style="font-size: 7.2pt; color: #475569; margin-bottom: 6px;">
            Le présent bordereau récapitule les convocations officielles émises pour la session disciplinaire en cours. Les candidats convoqués sont tenus d'émarger ci-après pour valoir notification légale conforme aux dispositions de la Loi 01-00.
        </div>
        ...
        <table class="bordereau-table">
            <thead>
                <tr>
                    <th style="width: 10%; text-align: center;">N° Dossier</th>
                    <th style="width: 26%;">Étudiant Poursuivi</th>
                    <th style="width: 20%;">CNE / Filière</th>
                    <th style="width: 22%;">Motif & Épreuve</th>
                    <th style="width: 22%; text-align: center;">Signature / Émargement Réception</th>
                </tr>
            </thead>
            <tbody>
                @foreach($items as $item)
                <tr>
                    <td style="text-align: center; font-family: monospace; font-weight: bold; color: #4a1010;">
                        CD-2026/{{ str_pad($item['incident']->id, 4, '0', STR_PAD_LEFT) }}
                    </td>
                    <td>
                        <strong>{{ strtoupper($item['fullNameFr']) }}</strong>
                        @if(!empty($item['fullNameAr']))
                            <div style="font-size: 6.5pt; color: #854d0e;">{{ $item['fullNameAr'] }}</div>
                        @endif
                    </td>
                    <td>
                        <div style="font-family: monospace; font-weight: bold;">{{ $item['cne'] }}</div>
                        <div style="font-size: 6.2pt; color: #64748b;">{{ $item['filiere'] }}</div>
                    </td>
                    <td>
                        <div style="color: #991b1b; font-weight: bold;">{{ $item['typeLabel'] }}</div>
                        <div style="font-size: 6.2pt; color: #475569;">{{ $item['module'] }}</div>
                    </td>
                    <td style="text-align: center; height: 32px; color: #94a3b8; font-style: italic; font-size: 6.5pt;">
        ...
        <table class="signatures-table" style="margin-top: 15px;">
            <tr>
                <td>
                    <div class="sign-role">Le Responsable de la Scolarité</div>
                    <div class="sign-sub">Service des Examens</div>
                    <div style="height: 40px;"></div>
                    <div style="font-size: 6.5pt; font-weight: bold; color: #475569;">(Cachet & Visa de Transmission)</div>
                </td>
                <td>
                    <div class="sign-role">Le Secrétaire Général de l'ENCG Fès</div>
                    <div class="sign-sub">Présidence de l'Établissement</div>
                    <div style="height: 40px;"></div>
                    <div style="font-size: 6.5pt; font-weight: bold; color: #475569;">(Signature & Approbation du Bordereau)</div>
                </td>
            </tr>
        </table>

        <div style="position: absolute; bottom: 8px; left: 14px; right: 14px; border-top: 1px dashed #cbd5e1; padding-top: 4px; font-size: 6pt; font-family: monospace; color: #94a3b8; text-align: center;">
            Sceau du Lot : {{ $batchSealHash ?? 'ENCG-BATCH-CD-2026' }} — Page 1 sur {{ count($items) + 1 }}
        </div>
    </div>

    <!-- ========================================================= -->
    <!-- PAGES 2..N : CONVOCATIONS INDIVIDUELLES A4 DE CHAQUE CANDIDAT -->
    <!-- ========================================================= -->
    @foreach($items as $item)
    ...
    <div class="page-container">
            ...
            <tr>
                ...
                <td style="width: 25%; text-align: right;">
                    <div style="font-size: 6.5pt; color: #64748b; font-family: monospace;">
                        ...
                        ENCG/CD-2026/{{ str_pad($item['incident']->id, 4, '0', STR_PAD_LEFT) }}<br>
                        Date : {{ date('d/m/Y') }}
                    </div>
                </td>
            </tr>
        ...
        <table class="dossier-table">
            <tr>
                <td class="dossier-cell" style="width: 50%;">
                    ...
                    <span class="tile-value">{{ strtoupper($item['fullNameFr']) }}</span>
                    @if(!empty($item['fullNameAr']))
                        <span style="font-size: 8.5pt; color: #854d0e; font-weight: bold; margin-left: 6px;">({{ $item['fullNameAr'] }})</span>
                    @endif
                </td>
                <td class="dossier-cell" style="width: 25%;">
                    <span class="tile-label">Code National / Massar</span>
                    <span class="tile-value" style="font-family: monospace;">{{ $item['cne'] }}</span>
                </td>
                <td class="dossier-cell" style="width: 25%;">
            ...
            <tr>
                <td class="dossier-cell" colspan="2">
                    ...
                    <span class="tile-value" style="font-size: 8pt; color: #4a1010;">{{ $item['filiere'] }}</span>
                </td>
            ...
            <table class="incident-table">
                <tr>
                    ...
                    <td class="incident-val" style="font-size: 8.2pt; color: #4a1010;">
                        ...
                        <span style="font-size: 7pt; color: #64748b;">(Date : {{ $item['examDate'] }})</span>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="incident-val" style="color: #854d0e; font-weight: bold;">
                        📦 {{ $item['confiscated'] }} (Placés sous scellé au secrétariat de séance)
                    </td>
                </tr>
                @endif
            </table>
        ...
        <div class="hearing-box">
            ...
            <div class="hearing-details">
                📅 Séance du : <u>{{ $item['hearingDate'] }}</u><br>
                📍 Lieu de réunion : <u>{{ $item['hearingRoom'] }}</u>
            </div>
        </div>
            ...
            <tr>
                <td style="width: 14%; text-align: left;">
                    @if(!empty($item['qrBase64']))
                        <img src="{{ $item['qrBase64'] }}" style="height: 48px; width: 48px;" alt="QR Authentification">
                    @endif
                </td>
                <td style="width: 86%; text-align: left; padding-left: 6px;">
                    <div class="hash-code">
                        ...
                        {{ $item['sealHash'] }}<br>
                        Document officiel certifié délivré par le Système Numérique Centralisé ENCG ERP.
                    </div>
                </td>
            </tr>
        </table>
