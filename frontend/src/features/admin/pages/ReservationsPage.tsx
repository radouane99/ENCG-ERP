...
export default function ReservationsPage() {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        setLoading(true)
        // Use the attendances endpoint as reservations (attendance sessions with room info)
                ...
                <tr key={res.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-slate-800">{res.room_name || res.room || 'N/A'}</div>
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{res.records_count || 0} participants</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 text-xs font-bold">
                        {(res.professor_name || 'P')[0]}
                      </div>
                      <div>
                        <div className="font-bold text-slate-800">{res.professor_name || 'N/A'}</div>
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{res.group_name || 'N/A'}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="font-bold text-slate-800 text-xs mb-1">{res.created_at?.split(' ')[0] || res.date || 'N/A'}</div>
                    <div className="text-[10px] text-slate-500 font-medium">{res.module_name || res.time || '-'}</div>
                  </td>
                  <td className="px-6 py-5">
                    <span className={cn(
                      "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider inline-flex items-center gap-1",
                      res.status === 'active' ? 'bg-emerald-100 text-emerald-600' : 
                      res.status === 'closed' ? 'bg-slate-100 text-slate-500' : 'bg-amber-100 text-amber-600'
                    )}>
                      <CheckCircle2 className="w-3 h-3" /> {res.status?.toUpperCase() || 'N/A'}
                    </span>
                  </td>
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-3">
                      <Link to={`/admin/reservations/${res.id}/edit`} className="text-amber-500 hover:bg-amber-50 p-2 rounded-lg transition-colors">
