...
export default function AdminGradesPage() {
  ...
  const selectClasses = "w-full rounded-2xl border bg-[var(--background)] px-12 py-4 text-sm font-bold transition-all outline-none appearance-none bg-no-repeat cursor-pointer focus:ring-2"
  const getSelectStyle = (hasValue: boolean) => cn(
    selectClasses,
    hasValue 
      ? "border-[var(--color-primary)/30] bg-[var(--color-primary)/5] text-[var(--foreground)] focus:border-[var(--color-primary)] focus:ring-[var(--color-primary)/20]" 
      : "border-[var(--border)] text-[var(--muted-foreground)] hover:border-[var(--border-hover)] focus:border-[var(--ring)] focus:ring-[var(--ring)/20]"
  )
  ...
  return (
    <div className="space-y-8 animate-in p-4 md:p-6 max-w-[1200px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[var(--foreground)]">{t('admin:grades.title')}</h1>
          <p className="text-[var(--muted-foreground)] mt-2">{t('admin:grades.subtitle')}</p>
        </div>
      </div>

      {/* Console Banner */}
      <div className="bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] p-8 text-white rounded-3xl shadow-xl shadow-[var(--color-primary)/10] relative overflow-hidden">
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl shadow-sm p-6 md:p-10 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-[var(--border)]">
          ...
          <div className={cn("space-y-4 transition-all duration-300", !filiere && "opacity-40 grayscale pointer-events-none")}>
            ...
            <div className="relative ps-11">
              <Users className={cn("w-5 h-5 absolute start-14 top-1/2 -translate-y-1/2 transition-colors", groupe ? "text-[var(--color-primary)]" : "text-[var(--muted-foreground)]")} />
              <select 
                value={groupe}
        ...
        {/* Action Bar */}
        <div className="pt-8 border-t border-[var(--border)] flex flex-col sm:flex-row justify-end gap-4">
          <Button
            size="lg"
            variant="outline"
          ...
          <Button
            ...
            className={cn("px-8 py-6 text-base font-bold shadow-lg transition-all", isFormComplete ? "hover:scale-105 text-white bg-[var(--color-primary)]" : "")}
          >
