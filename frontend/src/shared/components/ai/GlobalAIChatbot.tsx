  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={cn(
            'z-50 bg-[var(--background)] border border-[var(--border)] shadow-2xl flex flex-col overflow-hidden transition-all duration-300',
            windowClass
          )}
        >
          {/* Header */}
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[var(--muted)]/30">
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                ...
                <div className={cn(
                  ...
                  <p className={cn('text-[10px] mt-1.5 font-medium', msg.role === 'user' ? 'text-violet-200' : 'text-[var(--muted-foreground)]')}>
            ...
            {isTyping && (
              <div className="flex gap-2.5 justify-start animate-in fade-in duration-200">
                ...
                <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm flex gap-1 items-center">
                  {[0, 150, 300].map(delay => (
                    <div
                      key={delay}
          ...
          {/* Quick Actions */}
          <div className="px-4 py-2.5 border-t border-[var(--border)] bg-[var(--card)] flex gap-2 overflow-x-auto shrink-0 scrollbar-hide">
          ...
          {/* Input */}
          <div className="p-3 bg-[var(--card)] border-t border-[var(--border)] shrink-0">
            <form
              onSubmit={(e) => handleSendMessage(e)}
              className="flex gap-2 items-center bg-[var(--muted)] p-1.5 rounded-2xl border border-[var(--border)] focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all"
            >
              <input
                ref={inputRef}
                type="text"
                ...
                disabled={isTyping}
                className="flex-1 bg-transparent px-3 py-2 text-sm font-medium focus:outline-none text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] disabled:opacity-50"
              />
              <button
                type="submit"
