...
export function cleanUtf8Text(str: string): string {
  ...
  return str
    ...
    .replace(/Comptabilit[^\w\s]*\s*G[^\w\s]*n[^\w\s]*rale\s*(I+)/gi, 'Comptabilité Générale $1')
    .replace(/Comptabilit[^\w\s]*\s*Approfondie/gi, 'Comptabilité Approfondie')
    .replace(/Comptabilit[^\w\s]*/gi, 'Comptabilité')
    .replace(/G[├┬|─\-\s\u0080-\u00FF]*n[├┬|─\-\s\u0080-\u00FF]*rale/gi, 'Générale')
    .replace(/Statistiques\s*et\s*Probabilit[^\w\s]*s/gi, 'Statistiques et Probabilités')
