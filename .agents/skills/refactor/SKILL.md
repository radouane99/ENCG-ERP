...
## Backend Specifics (PHP 8.4 & Laravel 12)

### 1. Thin Controllers & Fat Models/Services
- **BAD:** Putting business logic, validation, or complex queries in the Controller.
- **GOOD:** Extract logic into dedicated Action classes, Service classes, or FormRequests. Controllers should only handle HTTP requests and return responses.

### 2. Eloquent & N+1 Query Prevention
- **Rule:** Always check for N+1 query smells during refactoring.
- **Fix:** Replace lazy loading inside loops with strict eager loading (`with()`) at the query level.

### 3. Modern PHP 8.4 Features
- Refactor old `switch` statements into `match` expressions.
- Use constructor property promotion and typed properties.
- Utilize asymmetric visibility (e.g., `public private(set) string $name;`) to enforce encapsulation instead of writing manual getters/setters.
## Frontend Specifics (React 19)

### 1. Custom Hooks for Logic Extraction
- **Rule:** If a functional component exceeds 100 lines due to `useEffect` or complex state management, extract that logic into a custom hook (e.g., `useStudentData`). Keep components focused on UI rendering.

### 2. Modern React 19 Patterns
- Refactor manual loading/error states using React 19's `useActionState` and `<form action={...}>` where applicable.
- Replace `useMemo` and `useCallback` where the React 19 compiler makes them redundant, focusing purely on clean functional logic.

## Database & Query Optimization

### 1. SARGable Queries
- **Rule:** When refactoring database queries (Eloquent or raw SQL), ensure the `WHERE` clauses are SARGable (Search Argument Able).
- **Fix:** Do not wrap indexed columns in functions (e.g., `WHERE YEAR(created_at) = 2026`). Refactor to use ranges (e.g., `WHERE created_at >= '2026-01-01'`).

## Testing Requirements

### 1. Testability First
- Refactored code must be easily testable. Avoid hardcoded dependencies; use dependency injection (e.g., passing interfaces through constructors in Laravel) so they can be mocked in Pest/PHPUnit.